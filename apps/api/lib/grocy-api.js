...
async function grocyFetch(method, path, body) {
  let baseUrl = config.grocy.baseUrl;
  const apiKey = config.grocy.apiKey;
  if (!baseUrl || !apiKey) {
    throw new Error('GROCY_API_URL and GROCY_API_KEY are required');
  }
  ...
  // Log errors
  if (!res.ok) {
    console.error(`[grocyFetch ERROR] ${method} ${url} -> ${res.status} ${res.statusText}`);
    if (text.length < 200) {
      console.error(`[grocyFetch ERROR] Response: ${text}`);
    }
