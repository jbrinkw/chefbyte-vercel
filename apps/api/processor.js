...
// Load env from Luna root .env (three levels up from ui/)
try { require('dotenv').config({ path: __dirname + '/../../../.env' }); } catch (_) {}

const BASE = (process.env.GROCY_API_URL || process.env.GROCY_BASE_URL || '').replace(/\/$/, '');
...
async function grocyFetch(method, path, body) {
  if (!BASE || !KEY) throw new Error('GROCY_API_URL and GROCY_API_KEY are required on the server');
    ...
    https.get(url, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          const brand = (product.brands || '').toString().trim();
          const productName = (product.product_name || '').toString().trim();
          
          // Priority 1: Per-serving data (when available)
          let calories = nutriments['energy-kcal_serving'];
          let carbs = nutriments.carbohydrates_serving;
          let fats = nutriments.fat_serving;
          let protein = nutriments.proteins_serving;
          let dataSource = 'per-serving';
          
          // Fallback: Per-100g data
          if (!Number.isFinite(calories) || !Number.isFinite(carbs) || !Number.isFinite(fats) || !Number.isFinite(protein)) {
          ...
          const servingQuantity = Number(product.serving_quantity) || 1;
          const productQuantity = Number(product.product_quantity) || null;
          
          // Calculate servings per container if possible
          let servingsPerContainer = 1;
          if (Number.isFinite(productQuantity) && Number.isFinite(servingQuantity) && servingQuantity > 0) {
            servingsPerContainer = Math.max(1, Math.round(productQuantity / servingQuantity));
          }
          
          resolve({
            brand_name: brand,
            food_name: productName,
            product_name: (brand && productName) ? `${brand} ${productName}` : (productName || brand || 'Unknown Product'),
            nf_calories: Number(calories) || 0,
            nf_total_carbohydrate: Number(carbs) || 0,
            nf_total_fat: Number(fats) || 0,
            nf_protein: Number(protein) || 0,
            serving_qty: servingsPerContainer,
...
async function aiSuggest(off) {
  if (!OPENAI_API_KEY) return null;
  try {
    const OpenAI = require('openai');
    const client = new OpenAI.OpenAI({ apiKey: OPENAI_API_KEY });
    const brand = (off && off.brand_name) ? String(off.brand_name).trim() : '';
    const food = (off && off.food_name) ? String(off.food_name).trim() : '';
...
async function overridePlaceholder(productId, offData, barcode, logs) {
  try {
    ...
    const food = (offData.food_name || '').toString().trim();
    ...
    if (ufMap['Calories per Serving']) userfieldUpdates[ufMap['Calories per Serving']] = Number(offData.nf_calories || 0);
    if (ufMap['Carbs']) userfieldUpdates[ufMap['Carbs']] = Number(offData.nf_total_carbohydrate || 0);
    if (ufMap['Fats']) userfieldUpdates[ufMap['Fats']] = Number(offData.nf_total_fat || 0);
    if (ufMap['Protein']) userfieldUpdates[ufMap['Protein']] = Number(offData.nf_protein || 0);
    if (ufMap['Number of Servings']) userfieldUpdates[ufMap['Number of Servings']] = Number(offData.serving_qty || 1);
    if (ufMap['Serving Weight (g)']) userfieldUpdates[ufMap['Serving Weight (g)']] = 0; // OFF doesn't provide serving weight in grams directly
    userfieldUpdates['placeholder'] = 0; // No longer a placeholder (Grocy checkboxes use 0/1 not false/true)
    ...
    const totalCals = Math.round((Number(offData.nf_calories || 0)) * (Number(offData.serving_qty || 1)));
...
async function processAdd(barcode, logs) {
  ...
  step(logs, 'Not found in Grocy; fetching Open Food Facts ...');
  const off = await fetchOpenFoodFactsByBarcode(barcode);
  if (!off) throw new Error('Open Food Facts had no item for this barcode');
  data(logs, 'openfoodfacts.item', off);

  // Check for placeholder match before creating new product
  const brand = (off.brand_name || '').toString().trim();
  const food = (off.food_name || '').toString().trim();
  ...
  if (placeholderMatch && Number.isFinite(placeholderMatch) && placeholderMatch > 0) {
    ...
    const finalProductId = await overridePlaceholder(placeholderMatch, off, barcode, logs);
  ...
  maybe('Calories per Serving', (typeof off.nf_calories === 'number') ? off.nf_calories : Number(off.nf_calories));
  maybe('Carbs', (typeof off.nf_total_carbohydrate === 'number') ? off.nf_total_carbohydrate : Number(off.nf_total_carbohydrate));
  maybe('Fats', (typeof off.nf_total_fat === 'number') ? off.nf_total_fat : Number(off.nf_total_fat));
  maybe('Number of Servings', (typeof off.serving_qty === 'number') ? off.serving_qty : Number(off.serving_qty));
  maybe('Protein', (typeof off.nf_protein === 'number') ? off.nf_protein : Number(off.nf_protein));
  maybe('Serving Weight (g)', 0); // OFF doesn't provide serving weight in grams directly
  ...
  return { status: 'ok', message: 'Created product via Open Food Facts+AI and added 1', barcode, operation: 'add', created_product: true, added_amount: 1, product_id, product_name: name };
}
